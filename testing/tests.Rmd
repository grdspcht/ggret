...
retnet3=ggarg::read.beast.retnet("../data/retnet.nexus")
#plot the network 
...
#We need to remove the % from the variable name otherwise it causes problem for geom_range
retnet.bacter@data$`height_95_HPD`=retnet.bacter@data$`height_95%_HPD`
#Define color of genotypes
col.clades=c(Undefined="#444444","Genotype A"="#CC0033","American branch"="#3366FF")
#define plot: changing the axis to BP scale doesn't work, nor plotting date ranges at the nodes
retnet.bacter.plot=ggret(retnet.bacter,ladderize = F,as.Date = F,root.position = -mrca,aes(color=clade),size=linesize.tree) + theme_tree2() +
  theme(legend.position="n", axis.text=element_text(size=fontsize.subgenotype*3), axis.title=element_text(size=fontsize.subgenotype*3),axis.line = element_line(linewidth = linesize.tree)) +
  scale_color_manual(values=col.clades) +
  geom_nodelab(aes(label=round(posterior,2)),size=fontsize.tips,hjust=1.5,vjust=-0.5) +
  geom_range(aes(range="height_95_HPD"), color="grey50", alpha=.6, size=fontsize.tips) + 
  #scale_x_continuous(breaks = xticks, labels=-xticks,
   #                  limits=c(-(max(retnet.bacter@data$`height_95%_HPD` %>% unlist,na.rm = T)+500),offset.genotype+2000)) +
  xlab("years BP") +
  scale_y_discrete(expand = c(expand_y,0.01)) +
  #geom_vline(xintercept = xticks,color="grey60",lty="longdash",size=linesize.vbars) +
  #geom_vline(xintercept = xticks[1:(length(xticks)-1)] + 1250,color="grey80",lty="dashed",size=linesize.vbars) +
...
#Here is an alternative code to change the axis to a BP scale
xticks_BP=-xticks
retnet.bacter.plot=ggret(retnet.bacter,ladderize = F,as.Date = F,root.position = -mrca,aes(color=clade),size=linesize.tree) + theme_tree2() +
  theme(legend.position="n", axis.text=element_text(size=fontsize.subgenotype*3), axis.title=element_text(size=fontsize.subgenotype*3),axis.line = element_line(linewidth = linesize.tree)) +
  scale_color_manual(values=col.clades) +
  geom_nodelab(aes(label=round(posterior,2)),size=fontsize.tips,hjust=1.5,vjust=-0.5) +
  #geom_range(aes(range="height_95_HPD", color="grey50"), alpha=.6, size=fontsize.tips) + doesn't work
  scale_x_continuous(breaks = mrca - xticks_BP, labels=xticks_BP,
                     limits=c(mrca-(max(retnet.bacter@data$`height_95%_HPD` %>% unlist,na.rm = T)+500),mrca + offset.genotype+2000)) +
  xlab("years BP") +
  scale_y_discrete(expand = c(expand_y,0.01)) +
  geom_vline(xintercept = mrca - xticks_BP,color="grey60",lty="longdash",size=linesize.vbars) +
  geom_vline(xintercept = (mrca - xticks_BP)[1:(length(xticks_BP)-1)] + 1250,color="grey80",lty="dashed",size=linesize.vbars) +
  geom_cladelab(node=mrca_american, label="American branch",textcolor=col.clades["American branch"], barcolor= col.clades["American branch"],align=F,fontsize = fontsize.genotype,offset=offset.genotype, offset.text = this.offset.text) +
  geom_cladelab(node=mrca_genA, label="Genotype A",textcolor=col.clades["Genotype A"], barcolor= col.clades["Genotype A"],align=F,fontsize = fontsize.genotype,offset=offset.genotype, offset.text = this.offset.text)
```
